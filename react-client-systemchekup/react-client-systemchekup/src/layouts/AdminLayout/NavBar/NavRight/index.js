...
const NavRight = () => {
    ...
    //alert(JSON.stringify(account.user.username));
    const userName=account.user.username;

    const [listOpen, setListOpen] = useState(false);

    const handleLogout = () => {
        //alert('logout info: '+API_SERVER + account.token + 'users/logout', {}, { headers: { Authorization: `${account.token}` } })
        axios
            ...
            .catch(function (error) {
                ...
                //alert('logout fail')
                //when token  expired then force logout
                dispatcher({ type: LOGOUT });
            });
    };
